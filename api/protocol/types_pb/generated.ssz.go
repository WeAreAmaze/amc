...
// Hash: e5beb96aebef05dfdaf3de6ddc8bde8cf6caacf8de6caa4cd918a624534b6585
package types_pb
...
func (t *Transactions) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	for ii := 0; ii < len(t.Transactions); ii++ {
		offset += 4
		offset += t.Transactions[ii].SizeSSZ()
	}

	// Field (0) 'Transactions'
	if size := len(t.Transactions); size > 536870912 {
		err = ssz.ErrListTooBigFn("--.Transactions", size, 536870912)
		return
	}
	{
	...
	for ii := 0; ii < len(t.Transactions); ii++ {
		if dst, err = t.Transactions[ii].MarshalSSZTo(dst); err != nil {
			return
		}
	}
...
func (t *Transactions) UnmarshalSSZ(buf []byte) error {
	...
	{
		buf = tail[o0:]
		num, err := ssz.DecodeDynamicLength(buf, 536870912)
		if err != nil {
			return err
		}
		t.Transactions = make([]*Transaction, num)
		err = ssz.UnmarshalDynamic(buf, num, func(indx int, buf []byte) (err error) {
			if t.Transactions[indx] == nil {
				t.Transactions[indx] = new(Transaction)
			}
			if err = t.Transactions[indx].UnmarshalSSZ(buf); err != nil {
				return err
			}
			return nil
		})
...
func (t *Transactions) SizeSSZ() (size int) {
	...
	for ii := 0; ii < len(t.Transactions); ii++ {
		size += 4
		size += t.Transactions[ii].SizeSSZ()
	}
...
func (t *Transactions) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	...
	// Field (0) 'Transactions'
	{
		...
		if ssz.EnableVectorizedHTR {
			hh.MerkleizeWithMixinVectorizedHTR(subIndx, num, 536870912)
		} else {
			hh.MerkleizeWithMixin(subIndx, num, 536870912)
		}
	}
