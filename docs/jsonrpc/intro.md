...
You can interact with amc over JSON-RPC. amc supports all standard Ethereum JSON-RPC API methods.

JSON-RPC is provided on multiple transports. amc supports HTTP, WebSocket and IPC (both UNIX sockets and Windows named pipes). Transports must be enabled through command-line flags.
...
amc supports HTTP, WebSockets and IPC.
...
To start an HTTP server, pass --http to amc node:
...
```bash
amc --http --http.corsdomain https://mycoolapp.rs
```

Alternatively, if you want to allow any domain, you can pass `*`:

```bash
amc --http --http.corsdomain "*"
```
...
Reth creates a UNIX socket on Linux and macOS at `/tmp/amc.ipc`. On Windows, IPC is provided using named pipes at `\\.\pipe\amc.ipc`.
