...
func TruncateRange(db kv.RwTx, bucket string, key []byte, to uint32) error {
	...
	defer c.Close()
	if err := kv.Walk(c, chunkKey, 0, func(k, v []byte) (bool, error) {
	...
	return WalkChunkWithKeys(key, bm, ChunkLimit, func(chunkKey []byte, chunk *roaring.Bitmap) error {
		...
		return db.Put(bucket, chunkKey, utils.Copy(buf.Bytes()))
	})
}
...
func TruncateRange64(db kv.RwTx, bucket string, key []byte, to uint64) error {
	...
	bm, err := Get64(db, bucket, key, to, MaxUint64)
	if err != nil {
		return err
	}
...
func SeekInBitmap64(m *roaring64.Bitmap, n uint64) (found uint64, ok bool) {
	...
	searchRank := m.Rank(n)
	if searchRank == 0 || searchRank == m.GetCardinality() {
		return 0, false
	}
	found, _ = m.Select(searchRank - 1)
	return found, true
}
