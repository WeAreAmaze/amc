...
type ChainHeaderReader interface {
	...
	GetHeader(hash types.Hash, number types.Int256) block.IHeader
...
type Engine interface {
	...
	Finalize(chain ChainHeaderReader, header block.IHeader, state *statedb.StateDB, txs []*transaction.Transaction,
	...
	// Note: The block header and state database might be updated to reflect any
	// consensus rules that happen at finalization (e.g. block rewards).
	FinalizeAndAssemble(chain ChainHeaderReader, header block.IHeader, state *statedb.StateDB, txs []*transaction.Transaction,
		uncles []block.IHeader, receipts []*block.Receipt) (block.IBlock, error)
	...
	CalcDifficulty(chain ChainHeaderReader, time uint64, parent block.IHeader) types.Int256
