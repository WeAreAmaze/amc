...
func (opts MdbxOpts) Open() (kv.RwDB, error) {
	...
	if !opts.inMem {
		if staleReaders, err := db.env.ReaderCheck(); err != nil {
			...
			log.Info("[db] cleared reader slots from dead processes", "amount", staleReaders)
		}
...
func (db *MdbxKV) Close() {
	...
	if db.opts.inMem {
		if err := os.RemoveAll(db.opts.path); err != nil {
			log.Warn("failed to remove in-mem db file", "err", err)
		}
	}
}
...
func (tx *MdbxTx) CollectMetrics() {
	...
	//todo Metrics
	_, err = tx.tx.Info(true)
	if err != nil {
		return
	}

	_, err = tx.BucketStat("gc")
	if err != nil {
		return
	}

	{
		_, err := tx.BucketStat(kv.PlainState)
		if err != nil {
			return
		}
	}
	{
		_, err := tx.BucketStat(kv.StorageChangeSet)
		if err != nil {
			return
		}
	}
	{
		_, err := tx.BucketStat(kv.EthTx)
		if err != nil {
			return
		}
	}
	{
		_, err := tx.BucketStat(kv.Log)
		if err != nil {
			return
		}
	}
