...
func (m *memoryMutationCursor) Current() ([]byte, []byte, error) {
	...
	return utils.Copy(m.currentPair.key), utils.Copy(m.currentPair.value), nil
}
...
func (m *memoryMutationCursor) Put(k, v []byte) error {
	return m.mutation.Put(m.table, utils.Copy(k), utils.Copy(v))
}

func (m *memoryMutationCursor) Append(k []byte, v []byte) error {
	return m.mutation.Append(m.table, utils.Copy(k), utils.Copy(v))

}

func (m *memoryMutationCursor) AppendDup(k []byte, v []byte) error {
	return m.memCursor.AppendDup(utils.Copy(k), utils.Copy(v))
}
