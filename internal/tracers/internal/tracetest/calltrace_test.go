...
type callTracerTest struct {
	Genesis      *core.GenesisBlock `json:"genesis"`
...
func testCallTracer(tracerName string, dirPath string, t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(file.Name(), ".json")), func(t *testing.T) {
			...
			if err := tx.UnmarshalBinary(common.FromHex1(test.Input)); err != nil {
				t.Fatalf("failed to parse testcase input: %v", err)
			}
			// Configure a blockchain with the given prestate
			var (
				signer    = transaction.MakeSigner(test.Genesis.GenesisBlockConfig.Config, new(big.Int).SetUint64(uint64(test.Context.Number)))
				...
				context = evmtypes.BlockContext{
					...
					BaseFee:     test.Genesis.GenesisBlockConfig.BaseFee,
				}
...
func benchTracer(tracerName string, test *callTracerTest, b *testing.B) {
	// Configure a blockchain with the given prestate
	tx := new(transaction.Transaction)
	if err := rlp.DecodeBytes(common.FromHex1(test.Input), tx); err != nil {
