...
func (m *memoryMutationCursor) Current() ([]byte, []byte, error) {
	...
	return types.CopyBytes(m.currentPair.key), types.CopyBytes(m.currentPair.value), nil
}
...
func (m *memoryMutationCursor) Put(k, v []byte) error {
	return m.mutation.Put(m.table, types.CopyBytes(k), types.CopyBytes(v))
}

func (m *memoryMutationCursor) Append(k []byte, v []byte) error {
	return m.mutation.Append(m.table, types.CopyBytes(k), types.CopyBytes(v))

}

func (m *memoryMutationCursor) AppendDup(k []byte, v []byte) error {
	return m.memCursor.AppendDup(types.CopyBytes(k), types.CopyBytes(v))
}
