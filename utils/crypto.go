...
import (
	...
	amc_crypto "github.com/amazechain/amc/common/crypto"
)
...
func ConvertFromInterfacePrivKey(privkey crypto.PrivKey) (*ecdsa.PrivateKey, error) {
	...
	privKey.D = k
	privKey.Curve = amc_crypto.S256() // Temporary hack, so libp2p Secp256k1 is recognized as geth Secp256k1 in disc v5.1.
	privKey.X, privKey.Y = amc_crypto.S256().ScalarBaseMult(rawKey)
	return privKey, nil
}
